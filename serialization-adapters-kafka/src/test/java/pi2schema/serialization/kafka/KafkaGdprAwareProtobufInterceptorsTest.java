  ...
  void shouldEncryptProtobufMessage() {
    ...
    ConsumerRecords<String, FarmerRegisteredEvent> messages = new ConsumerRecords<>(
        Map.of(new TopicPartition("topic", 0), List.of(
            new ConsumerRecord<>("topic", 0, 1, encryptedMessage.key(),
                encryptedMessage.value()))));

    Iterable<ConsumerRecord<String, FarmerRegisteredEvent>> decryptedMessages = consumerInterceptor.onConsume(messages)
        .records("topic");
    assertThat(decryptedMessages).hasSize(1);
    assertThat(decryptedMessages).first().satisfies(r -> {
