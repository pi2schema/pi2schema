    ...
    void shouldSupportAvro() {
        ...
        var messages = new ConsumerRecords<>(
            Map.of(
                ...
                List.of(new ConsumerRecord<>("topic", 0, 1, encryptedAvroMessage.key(), encryptedAvroMessage.value()))
            )
        );

        var decryptedMessages = consumerInterceptor.onConsume(messages).records("topic");
        assertThat(decryptedMessages).hasSize(1);
        assertThat(decryptedMessages)
            .first()
            .satisfies(r -> assertThat(r.value().getEmail()).isEqualTo("john.doe@email.com"));
    }
}
