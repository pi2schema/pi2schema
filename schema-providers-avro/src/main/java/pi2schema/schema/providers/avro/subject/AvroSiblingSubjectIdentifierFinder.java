...
public class AvroSiblingSubjectIdentifierFinder
    implements SubjectIdentifierFinder<AvroUnionPersonalDataFieldDefinition> {

    @Override
    public SubjectIdentifierFieldDefinition<SpecificRecordBase> find(
        AvroUnionPersonalDataFieldDefinition fieldDescriptor
    ) {
        var subjectsIdentifierFields = fieldDescriptor
            .getParentSchema()
            .getFields()
            .stream()
    ...
    private static boolean isSubjectIdentifierField(Schema.Field f) {
        return Optional
            .ofNullable(f.getObjectProp(SUBJECT_IDENTIFIER_PROPERTY_NAME))
            .map(Object::toString)
